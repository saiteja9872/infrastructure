...
// preprod01.naw03.jenkins.viasat.com/job/Mach3_Jenkins/job/Mach3_UT/job/UT_DEV/job/build_fsbl
        ...
        withCredentials (
        ...
        )
            {
                sh '''echo "Workspace is ${WORKSPACE}"
                    ...
                    echo -n "${ROM_DIR}/fsbl" > fsbl_path_param.txt
                    ...
                    # Use to differentiate FSBL builds with the same version
                    export JENKINS_BUILD_NUMBER="_${BUILD_NUMBER}"

                    cd ${ROM_DIR}

                    make fsbl_archive BLD_REL=cicd BLD_NO=1'''
            }
    }

    stage("update build name") {
        def buildVersion = readFile (
                file: 'Mach3/Technical/ASIC/ViaPHY3/firmware/SecureBootROM/fsbl/fsbl_version.txt'
            )
        currentBuild.displayName = "#${BUILD_NUMBER}_" + buildVersion
    }

    stage("archive artifacts"){
        archiveArtifacts 'Mach3/Technical/ASIC/ViaPHY3/firmware/SecureBootROM/fsbl/archive/**/*'
    }

    stage('kick off build_ssbl job') {
        sh "pwd"
        sh "ls -la"
        def fsbl_path_param = readFile (file: 'fsbl_path_param.txt')
        build (
            job: 'build_ssbl',
            parameters: [string (name: 'FSBL_PATH_PARAM', value: fsbl_path_param),
