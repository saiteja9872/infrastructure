...
./target/linux/viaphy3/prepare.sh -e blackbird dev-g
...
echo "Current UT version is $MAJOR.$MINOR.$MICRO.$NANO.$NEW_BUILD"
UT_VERSION=$MAJOR.$MINOR.$MICRO.$NANO.$NEW_BUILD
echo "UT_VERSION=$MAJOR.$MINOR.$MICRO.$NANO.$NEW_BUILD" > version.out
NEW_BUILD_NUMBER=`expr $BUILD_NUMBER + 8000`
echo "UT_VERSION_LABEL=$NEW_BUILD_NUMBER" >> version.out

# Check if new build version is valid
let newBuild=$BUILD+1;
versionIsValid $newBuild $BUILD 65535
...
editProductVersion $MAJOR $MINOR $MICRO $NANO $BUILD $FILENAME
...
DESCRIPTION="Jenkins changed version to UT.$MAJOR.$MINOR.$MICRO.$NANO.$BUILD	See $BUILD_URL for details"
